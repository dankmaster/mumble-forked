...
param(
	...
	[Parameter(Mandatory = $true)]
	[string]$LowPerformanceNightlyEvidenceRoot,

	[Parameter(Mandatory = $true)]
	[string]$MainstreamNightlyEvidenceRoot,
	...
	[Parameter(Mandatory = $true)]
	[ValidatePattern('^[1-9][0-9]*$')]
	[string]$NightlyQualityWorkflowRunId,
...
$masterRunnerEvidence = New-Object System.Collections.Generic.List[object]
$nightlyRunnerEvidence = New-Object System.Collections.Generic.List[object]
$sharedCorpusSha256 = ""
$sharedRecipeSetVersion = ""
$sharedModelHashes = @()
$sharedLegacyBinarySha256 = ""
$sharedProtectedBuildIdentity = ""
$suiteInputIdentity = @{}
$hardwareFingerprintByClass = @{}
foreach ($runner in @(
		@{ Suite = "master_quality"; MinimumCases = 500; Class = "low-performance"; Root = $LowPerformanceEvidenceRoot; RunId = $QualityWorkflowRunId },
		@{ Suite = "master_quality"; MinimumCases = 500; Class = "mainstream"; Root = $MainstreamEvidenceRoot; RunId = $QualityWorkflowRunId },
		@{ Suite = "nightly"; MinimumCases = 5000; Class = "low-performance"; Root = $LowPerformanceNightlyEvidenceRoot; RunId = $NightlyQualityWorkflowRunId },
		@{ Suite = "nightly"; MinimumCases = 5000; Class = "mainstream"; Root = $MainstreamNightlyEvidenceRoot; RunId = $NightlyQualityWorkflowRunId }
	)) {
	...
	Assert-ExactProperties -Object $harnessProvenance -Names @(
		"hardwareFingerprintSha256", "harnessFileName", "harnessSha256", "kind", "qualityWorkflowRunId",
		"runnerClass", "schemaVersion", "sourceSha"
	...
	if ([int]$harnessProvenance.schemaVersion -ne 1 -or
		...
		[string]$harnessProvenance.qualityWorkflowRunId -cne $runner.RunId -or
		[string]$harnessProvenance.runnerClass -cne $runner.Class -or
		[string]$harnessProvenance.harnessSha256 -cne $ExpectedHarnessSha256 -or
		[string]$harnessProvenance.hardwareFingerprintSha256 -cnotmatch '^[0-9a-f]{64}$') {
	...
	if ($qualitySha -cne $expectedSha -or
		[string](Assert-ObjectProperty -Object $quality -Name "qualification_scope" -Context "$($runner.Class) quality") -cne "core" -or
		[string](Assert-ObjectProperty -Object $quality -Name "suite" -Context "$($runner.Class) quality") -cne $runner.Suite -or
		[string](Assert-ObjectProperty -Object $quality -Name "status" -Context "$($runner.Class) quality") -cne "passed" -or
		[int](Assert-ObjectProperty -Object $coverage -Name "case_count" -Context "$($runner.Class) quality coverage") -lt $runner.MinimumCases) {
		throw "Measured evidence for '$($runner.Suite)/$($runner.Class)' does not attest a passing $($runner.MinimumCases)-case run for '$expectedSha'."
	}
	...
	$protectedBuildIdentity = [ordered]@{}
	foreach ($field in @(
		...
		"metrics_runtime_sha256", "model_manifest_sha256",
		"recipe_manifest_sha256", "recipe_set_version"
	)) {
		$value = [string](Assert-ObjectProperty -Object $build -Name $field -Context "$($runner.Class) quality build")
		if ($field -ne "recipe_set_version" -and $value -cnotmatch '^[0-9a-f]{64}$') {
			throw "Measured evidence for '$($runner.Class)' has an invalid '$field'."
		}
		$protectedBuildIdentity[$field] = $value
	}
	$protectedBuildIdentity["model_hashes"] = $modelHashes
	$protectedIdentityJson = $protectedBuildIdentity | ConvertTo-Json -Depth 8 -Compress
	if ([string]::IsNullOrWhiteSpace($sharedProtectedBuildIdentity)) {
		$sharedProtectedBuildIdentity = $protectedIdentityJson
	} elseif ($protectedIdentityJson -cne $sharedProtectedBuildIdentity) {
		throw "Protected runners did not test the same complete payload and provenance bundle."
	}
	$perSuiteIdentity = [ordered]@{
		mixturePlanSha256 = [string](Assert-ObjectProperty -Object $build -Name "mixture_plan_sha256" -Context "$($runner.Class) quality build")
		caseSetSha256     = [string](Assert-ObjectProperty -Object $build -Name "case_set_sha256" -Context "$($runner.Class) quality build")
	}
	$perSuiteIdentityJson = $perSuiteIdentity | ConvertTo-Json -Compress
	if (-not $suiteInputIdentity.ContainsKey($runner.Suite)) {
		$suiteInputIdentity[$runner.Suite] = $perSuiteIdentityJson
	} elseif ([string]$suiteInputIdentity[$runner.Suite] -cne $perSuiteIdentityJson) {
		throw "Protected runners did not use the same mixture plan and case set for '$($runner.Suite)'."
	}
	$hardwareFingerprint = [string]$harnessProvenance.hardwareFingerprintSha256
	if (-not $hardwareFingerprintByClass.ContainsKey($runner.Class)) {
		$hardwareFingerprintByClass[$runner.Class] = $hardwareFingerprint
	} elseif ([string]$hardwareFingerprintByClass[$runner.Class] -cne $hardwareFingerprint) {
		throw "Runner class '$($runner.Class)' changed its hardware/environment fingerprint between master and nightly."
	}
	...
	} elseif ($corpusSha256 -cne $sharedCorpusSha256 -or $recipeSetVersion -cne $sharedRecipeSetVersion -or
...
$document = [ordered]@{
	...
	protectedBuildIdentity = ($sharedProtectedBuildIdentity | ConvertFrom-Json)
	...
	masterInputIdentity    = ([string]$suiteInputIdentity["master_quality"] | ConvertFrom-Json)
	nightlyInputIdentity   = ([string]$suiteInputIdentity["nightly"] | ConvertFrom-Json)
	runners                = $masterRunnerEvidence.ToArray()
	nightlyRunners         = $nightlyRunnerEvidence.ToArray()
	qualityWorkflowRunId   = $QualityWorkflowRunId
	nightlyQualityWorkflowRunId = $NightlyQualityWorkflowRunId
	createdAtUtc           = (Get-Date).ToUniversalTime().ToString("o")
}
Write-ReleaseJson -Value $document -Path $OutputPath
Write-Host "Bound master-quality and nightly evidence from both protected runner classes to candidate '$testedBinarySha256' and legacy '$sharedLegacyBinarySha256'."
