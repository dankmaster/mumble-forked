...
$checks = [ordered]@{
	...
	webengine_quick_outside_media_allowlist = Find-Matches @('-n', 'QtWebEngine|WebEngineView|WebEngineProfile', 'src/mumble', '--glob', '*.cpp', '--glob', '*.h', '--glob', '*.qml', '--glob', '!MediaSessionWindow.qml', '--glob', '!InlineMediaPlayer.qml', '--glob', '!QmlMediaProfileFactory.cpp', '--glob', '!QmlMediaProfileFactory.h', '--glob', '!main.cpp')
