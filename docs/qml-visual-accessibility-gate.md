...
The checked-in Windows reference artifacts are tracked in `qml-visual-baseline/`. The Windows shared-client workflow builds a separate automation-enabled client only after publishing the release payload, then compares all matrix cases against that baseline. The shipped client and installer remain automation-disabled. This fixture gate does not replace connected interaction testing or human parity review.
