...
function probeScript(provider, adaptiveExpected) {
	...
	return "(function(){"
		...
		+ "spotify:'[data-testid=\"embed-widget\"],[data-testid=\"play-button\"],button[aria-label*=\"Play\"]',"
		+ "facebook:'[data-testid*=\"video\"],video',"
		+ "tiktok:'[data-e2e=\"embed-video\"],video',"
		+ "instagram:'article video,article [role=\"button\"],video',"
		+ "soundcloud:'.playControls,.widget,button[title*=\"Play\"],button[aria-label*=\"Play\"]',"
		+ "direct:'video,audio'};"
		...
		+ "const challengeText=/(confirm (that )?you('| a)?re not a bot|sign in to confirm|verify (that )?you are human|unusual traffic|bekräfta att du inte är en bot|logga in för att bekräfta|captcha)/.test(lower);"
		+ "const signInText=/(sign in|log in|logga in|sign-in required|login required)/.test(lower);"
		+ "const consentRoots=[document];const tiktokConsent=document.querySelector('tiktok-cookie-banner');if(tiktokConsent&&tiktokConsent.shadowRoot)consentRoots.push(tiktokConsent.shadowRoot);"
		+ "const consentAction=consentRoots.some(function(root){return Array.from(root.querySelectorAll('button,[role=\"button\"]')).some(function(node){const label=String(node.innerText||node.textContent||node.getAttribute('aria-label')||'').replace(/\\s+/g,' ').trim().toLowerCase();return /^(allow all( cookies)?|accept all( cookies)?|decline optional cookies|reject optional cookies|only allow essential cookies|tillåt alla( cookies)?|godkänn alla( cookies)?|avvisa valfria cookies|endast nödvändiga cookies)$/.test(label);});});"
		...
		+ "else if(consentAction)blockedKind='consent';"
		+ "else if(explicitError||unavailableText)blockedKind='unavailable';"
		+ "let paused=true,position=0,duration=0;"
		+ "if(youtubeTransport){const state=Number(yt.getPlayerState());paused=state!==1;position=Number(yt.getCurrentTime&&yt.getCurrentTime()||0);duration=Number(yt.getDuration&&yt.getDuration()||0);}"
